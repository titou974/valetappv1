...
const TicketShow = () => {
  ...
  return (
    <ClientLayout>
        <Navbar subtitle="Bienvenue au" title={ticketData?.restaurant.name} isLoading={isTicketLoading} position='static'/>
        <Ticket isTicketLoading={isTicketLoading} ticketData={ticketData} companyData={companyData} />
        <FooterBarLayout>
          <Button onClick={(e) => setEmailModal(true)} className='fill-primary-foreground' size="lg" color="primary" variant="solid" radius='full' fullWidth={true} endContent={< EnvelopeIcon width={20}/>}>
            Recevoir par email
          </Button>
        </FooterBarLayout>
        <div>
