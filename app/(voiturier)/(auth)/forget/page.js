...
const ForgetPassword = () => {
  ...
  const resetPassword = async (e) => {
    ...
    if (!siteData) {
      toast.error(
        'Votre lien de réinitialisation est invalide. Contactez le support'
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (data.message === 'Invalid phone number received.') {
        ...
        data.message === "User doesn't exists with this phone number."
      ) {
    ...
    <VoiturierLayout>
      ...
      <FooterBarLayout fixed={false}>
        ...
        <Button
          onClick={() =>
            router.push(
              `sign-in${siteData === null ? '' : `?site=${siteData?.id}`}`
            )
          }
          color='primary'
          variant='light'
      ...
      <LoadingModal
        ...
        title='Recherche de votre numéro...'
      />
    </VoiturierLayout>
  );
};
