  ...
  return (
    ...
    <div className="bg-black w-full text-white">
      <div className={`${styles.padding} flex flex-col justify-between h-screen`} >
        <div>
          <h3 className={styles.subText}>Bravo {sessionData?.user.name}</h3>
          <h2 className={styles.headText}>À bientôt 👋</h2>
        </div>
        <div>
          <p className={`text-center text-base py-2`}>Vous avez travaillé pendant <span className="font-bold">{sessionTime}</span></p>
          <p className={`text-center text-base ${styles.subText} py-2`}>au <span className="font-bold italic">{sessionData?.restaurant.name}</span></p>
        </div>
        <div className="flex flex-col justify-between gap-5">
          <Link href={`sign-in`} className="bg-primary w-full py-3 rounded-full flex items-center justify-center gap-2 hover:bg-white transition-colors">
