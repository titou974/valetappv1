      ...
      <Listbox value={input} onChange={setInput}>
        <div className="relative mt-1">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="z-50 absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-[20px]">
              {db?.map((site, siteIdx) => (
                <Listbox.Option
                  key={siteIdx}
                  className={({ active }) =>
                    `relative cursor-default select-none py-2 px-[20px] ${
